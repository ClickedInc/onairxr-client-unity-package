    ...
    public abstract class AirVRTrackerFeedbackBase : MonoBehaviour {
        ...
        public void Configure(AirVRProfileBase profile, GameObject bodyModelPrefab, PointerDesc pointerDesc) {
            ...
            if (_pointerDesc.enabled) {
                Material mat = new Material(Shader.Find("onAirXR/Unlit vertex color"));
        ...
        public void OnPreConnect(AirVRProfileBase profile) {
        ...
        private void Start() {
            createCookie(Shader.Find("onAirXR/Unlit alpha blended"));
        ...
        private void LateUpdate() {
            shouldRenderVisuals = srcDeviceConnected && AirVRInputManager.inputStream.GetState((byte)srcDevice, (byte)AXRHandTrackerFeedbackControl.RenderOnClient) != 0;
            if (shouldRenderVisuals) {
                AirVRInputManager.inputStream.GetRaycastHit((byte)srcDevice,
                                                            (byte)AXRHandTrackerFeedbackControl.RaycastHit,
                                                            ref _rayOrigin,
        ...
        private void updateVibration() {
            ...
            if (_vibrationTicker.tickOnFrame) {
                float frequency = 0, amplitude = 0;
                if (AirVRInputManager.inputStream.GetVibrationFrame((byte)srcDevice, (byte)AXRHandTrackerFeedbackControl.Vibration, ref frequency, ref amplitude)) {
